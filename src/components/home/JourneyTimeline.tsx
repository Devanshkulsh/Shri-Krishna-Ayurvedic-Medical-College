    ...
    <section className="relative overflow-hidden bg-white py-24">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="mt-2 text-3xl font-bold text-dark md:text-5xl">Journey of Healing</h2>
            ...
            {timelineEvents.map((event, idx) => (
              <div key={idx} className="relative flex flex-col md:flex-row items-center justify-between">
                ...
                <motion.div
                  ...
                  viewport={{ once: true, margin: "-100px" }}
                  className={`flex w-full flex-col md:w-5/12 ${idx % 2 === 0 ? 'md:text-right' : 'md:ml-auto md:text-left'} bg-white p-6 rounded-2xl shadow-sm border border-gray-50`}
                >
                   <div className="md:hidden inline-block mb-3 rounded-full bg-primary/10 px-3 py-1 text-sm font-bold text-primary w-fit">
                    {event.year}
                  </div>
                  <h3 className="text-2xl font-bold text-dark">{event.title}</h3>
